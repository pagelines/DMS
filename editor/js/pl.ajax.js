	...
	$.plAJAX = {
		...
		run: function( args ){
			...
			,	theData = {
					...
					,	log: false
					,	confirm: false
					, 	confirmText: pl_i18n.gettext( "Are you sure?" )
					,	savingText: 'Saving'
					,	refresh: 	false
					,	refreshText: pl_i18n.gettext( "Refreshing page..." )
		...
		, saveData: function( opts ){
			...
			var args = {
					mode: 'save'
				,	savingText: pl_i18n.gettext("Saving Settings")
				,	refresh: false
				,	refreshText: pl_i18n.gettext("Settings successfully saved! Refreshing page...")
				, 	log: true
				,	pageData: $.pl.data
				,	run: 'draft'
			}
		...
		, toggleEditor: function(){
			...
			confirmText = pl_i18n.gettext( "<h3>Turn Off DMS Editor?</h3><p>(Note: Draft mode is disabled when editor is off.)</p>" )
			bootbox.confirm( confirmText, function( result ){
				if(result == true){
					$.ajax( {
						...
						, data: theData
						, beforeSend: function(){
							bootbox.dialog( that.dialogText(pl_i18n.gettext( "Deactivating...") ), [], {animate: false})
						}
						, success: function( response ){


							bootbox.dialog( that.dialogText(pl_i18n.gettext( "Editor deactivated! Reloading page.") ), [], {animate: false})
		...
		, bindUIActions: function(){
			...
			$('.btn-revert').on('click.revertbutton', function(e){
					...
					,	args = {
							...
							,	run: 'revert'
							,	confirm: true
							,	confirmText: sprintf( pl_i18n.gettext( "<h3>Are you sure?</h3><p>This will revert <strong>%s</strong> changes to your last published configuration.</p>" ), revert )
							,	savingText: pl_i18n.gettext( "Reverting Draft" )
							,	refreshText: pl_i18n.gettext( "Template successfully updated!" )
							,	refresh: true
							, 	log: true
							,	revert: revert
						}

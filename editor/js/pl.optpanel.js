    ...
    $.optPanel = {
        ...
        , runEngine: function( opts, tabKey ){
            ...
            $.each( opts , function(index, o) {
                ...
                if(o.sortable)
                    sortableItem = true
                that.config.hasSortableItem = true
                ...
                {
                    //console.log(o)
                    optionHTML += "<input type='hidden' name='"+ o.name.split("[")[0]+"["+o.key+"_sortable]' class='lstn' value='' />"
                }

                out += sprintf( '<div class="opt opt-%s %s" data-number="%s"><div class="opt-name">%s</div><div class="opt-box%s">%s</div></div>', o.key, specialClass, number, theTitle, ((sortableItem) ? ' ui-section-sortable' : ''), optionHTML )
        ...
        , scriptEngine: function( tabIndex, o ) {
            ...
            // initialize sorting and hidden field to store sort order
            if( that.config.hasSortableItem) {
                console.log('initializing sorting');

                $('.ui-section-sortable').each(function() {
                    var ids = new Array();
                    $(this).find('.input-wrap').each( function() {
                        // handles input elements
                        $(this).find("input[id]:not([id$='-flip'])").each(function() {
                            ids.push( $(this).attr('id') );
                        });
                        // handles textarea and select elements
                        $(this).find("textarea,select").each(function() {
                            ids.push( $(this).attr('id') );
                        });
                    });
                    $(this).find('input[name$="_sortable]"]').attr("value", ids);
                });

                $('.ui-section-sortable').sortable(
                    {
                        axis:"y",
                        stop: function( event, ui ) {
                            var ids = new Array(), $thisparent = $(this).parent();
                            // loop through the panel's input wrappers to find the form fields that aren't tied to revers/flipp/ing checkboxes and store their ID in an array
                            $thisparent.find('.input-wrap').each( function() {
                                    // handles input elements
                                    $(this).find("input[id]:not([id$='-flip'])").each(function() {
                                        ids.push( $(this).attr('id') );
                                    });
                            ...
                            $thisparent.find('input[name$="_sortable]"]').attr("value", ids)
                ...
                that.config.hasSortableItem = false;
            }
